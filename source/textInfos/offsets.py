...
class OffsetsTextInfo(textInfos.TextInfo):
	...
	def _get_boundingRect(self):
		...
		try:
			...
			# Therefore, we need to create a bounding rectangle with points, which is less accurate but acceptable.
		...
		try:
			...
			# Ignore a LookupError here, as it could be thrown
			# if no reliable last visible offset can't be retrieved.
			pass
		...
		# Note that the location of the start offset is used when the range is collapsed.
		# This behavior is similar to UIA and MS Word.
		...
		return boundingRect.intersection(objLocation)
	...
	def _getPointsForRange(self,start,end):
		"""Retrieve the points for the characters in a given offset range.
		@param start: The start offset.
		@type start: int
		@param end: The end offset (exclusive).
		@type end: int
		@return: The points for the characters in the requested range.
		@rtype: [textInfos.Point]
		"""
		return [self._getPointFromOffset(offset) for offset in xrange(start, end)]
